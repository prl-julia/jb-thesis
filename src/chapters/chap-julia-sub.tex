...
and discuss its undecidability (\secref{sec:julia-sub:undec}).

\section{Overview of Subtyping}\label{sec:julia-sub:overview}
%% ======================================================================

The syntax of Julia types is given in \figref{fig:jltypes}.
...
program tries to access type variable \cjl{T}:
\begin{codeenvd}
\begin{julia}
...
\]
In the derivation variable \T is introduced by~\RLJ{R\_Intro} rule---before
...
Thus, when \RLJ{L\_Intro} discharges~\S, the variable is removed from 
...
quantification the form of the rule, the \RLJ{R\_Right} rule does not apply
...
Matching R-L
variables is specially dealt with by the \RLJ{R\_L} rule, which
...
% variables that would guarantee both the termination of constraint resolution
...
%   that would allow for complete constraint resolution.
% }\end{quotation}
%
% T <: Ref{Q}
% Ref{S} <: T
