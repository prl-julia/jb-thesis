...
As discussed in \chapref{sec:julia-sub:undec}, it is the interaction of
...
on this sublanguage first, in \chapref{sec:dec-sub:defs}.
In \chapref{sec:dec-sub:ext}, I extend the core language with nominal subtyping
and the diagonal rule.

The decidability of subtyping, %for the core language 
discussed in detail in \chapref{sec:dec-sub:proofs}, is achieved by
...
on page~\pageref{item:remove-uni-vars} (\chapref{sec:julia-sub:undec}).
...
and \tyinv\iname{\ty_1,\ldots} is a shorthand for the tightly-bound existential
...
For simplicity of presentation, I assume the following variable names 
...
Note that the constraints resolution algorithm does not need to compute
