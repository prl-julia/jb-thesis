...
}, which I processed with \code{JuliaPkgsList.jl}\footnote{
...
\begin{itemize}
    \item method type signatures: for example, 
      \cjl{f(x::T, v::Vector\{T\}) where T} is extracted as
...
\begin{itemize}
  \item For each bound variable, the analysis records information about the
    ...
    For example, in the type \cjl{Tuple\{Ref\{T\}, S where S\} where T},
    variable \cjl{T} is bound in \cjl{[]} and occurs in \cjl{[Tuple, Ref]};
...
an example of a parsing error is given in \figref{fig:evaluation-parse-errors}.
...
\begin{itemize}
  ...
  \item \numprint{22396} were partially processed: this happened when all 
...
question---the top \cjl{Dict} type on the right of
...
Additionally, I extracted and analyzed type declarations.
...
semantic false positive \cjl{Dict\{...Union\{T,Missing\}...\}}.
...
In general, in case where an equivalent representable type does not exist,
...
struct EqDict{T}
  d :: Dict{T,T}
    ...
    \cjl{Ref\{EqDict\{<:Any\}\}} can be used, but the data needs to wrapped.
    %All sites 
...
The first successful migration was done for \code{Muon.jl}:
...
involves \cjl{Type} and dispatch on type values.
