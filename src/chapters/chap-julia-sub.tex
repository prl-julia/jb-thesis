...
The type on the left denotes the set of all vectors of
vectors of elements of some type; the type on the right
requires a common type for all inner
...
and instead use a simpler $t <: t'$ notation,
...
\begin{enumerate}
  \item Because the right-hand side type is an existential,
    ...
    to subtyping of
    \[ 
      \tupletyp{\vectyp{\wheret{\t'^\S}{l'{<:}\S{<:}u'}}, t'}
        \ <:\ 
  ...
  \item Both sides of the subtyping query are now tuples of the matching length.
    ...
    \[ 
      \vectyp{\wheret{\t'^\S}{l'{<:}\S{<:}u'}}\ <:\ \vectyp{t_1^{\T}} 
      \ \ \text{and}\ \ t' <: t_2^{\T}.
    \]
...
% ^-----------|
%
Therefore, instead, I suggest \textbf{eliminating mutually constraining
...
\begin{enumerate}
  ...
  \item\label{item:remove-uni-vars} 
    ...
    Thus, eliminating variable-introducing existential types inside 
