...
on this sublanguage first, in \secref{sec:dec-sub:defs}.
In \secref{sec:dec-sub:ext}, I extend the core language with nominal subtyping
and the diagonal rule.

The decidability of subtyping, %for the core language 
discussed in detail in \secref{sec:dec-sub:proofs}, is achieved by
...
on page~\pageref{item:remove-uni-vars} (\secref{sec:julia-sub:undec}).
...
As I show in \chapref{chap:eval}, very few type annotations in a corpus of
...
% As outlined in~\chapref{sec:julia-sub:undec}
...
% entirely. Instead, they can be \emph{restricted} to the type grammar
% of {Java wildcards}~\cite{bib:torgersen:wildcards:2004}.
...
\begin{itemize}
    \item \secref{sec:dec-sub:defs} provides a complete definition
    ...
    \item \secref{sec:dec-sub:proofs} examines several properties
        of subtyping, in particular, decidability of subtyping
        (\thmref{thm:subtysig-terminates}) and
        soundness of constraint resolution (\thmref{thm:solvectr-sound}).
    \item \secref{sec:dec-sub:ext} extends the core language with 
...
and \tyinv\iname{\ty_1,\ldots} is a shorthand for the tightly-bounded
existential \tyinv\iname{\rexvarbound{\ty_1}{\ty_1},\ldots}.
...
along with the constraint resolution algorithm in \figref{fig:ctr-solve}.
...
{ 
    ...
    { 
        ...
        { 
            \inferrule*[right=\footnotesize\RSS{Types}]
            { \text{<discussed below>} }
...
discussed in \secref{sec:julia-sub:undec}.
...
\begin{itemize}
    \item unification-free subtyping of types \subtydflt{\ty}{\ty'}
        (\secref{subsec:dec-sub:subty});
    \item constrained subtyping of types \subtyctrdflt{\ty}{\ty'}
        (\secref{subsec:dec-sub:subtyctr});
    \item constraint resolution \solvectrdflt 
        (\secref{subsec:dec-sub:subtysig});
    \item signature subtyping \subtysigdflt{\tysig}{\tysig'}
        (\secref{subsec:dec-sub:subtysig}).
\end{itemize}

For simplicity of presentation, I assume the following variable name
convention based on Barendregt's convention:

\begin{definition}[Variable name convention]\label{def:var-names}
...
signature subtyping \subtysigdflt{\tysig}{\tysig'} and constrained subtyping
\subtyctrdflt{\ty}{\ty'}, variable bindings in $\tysig/\ty$ and $\tysig'/\ty'$
...
covariant tuples distribute over union types (\RST{UnionLeft}), 
...
\RST{Bot} should be written as \subtydflt{\tybot}{\ty'}; the change does not
...
if the unification variable \va stands for the union \tyunion{\va_1}{\va_2}
such that, distributively, \plug\dctx{\va_1} and \plug\dctx{\va_2}
...
\end{mathpar}
\end{minipage}}
Next, let us focus on the left premise and apply the same rule to
the remaining \va:\\
\makebox[\textwidth][c]{
\begin{minipage}{\ruleswidth}
\begin{mathpar}
...
Note that the constraint resolution algorithm does not need to compute
...
\begin{mathpar}
    ...
    { \tyunfvlddflt{\typair{\ty_1}{\ty_2}} }

    \inferrule*[]
    { \iname \text{ has arity } n \and
      \forall i \in 1..n.\ \tyunfvlddflt{\rexvar_i} }
    { \tyunfvlddflt{\tyinv\iname{\rexvar_1,\ldots,\rexvar_n}} }
...
\begin{itemize}
    ...
    \item \subtysigdflt{\tysig}{\tysig'} requires that \tyvlddflt{\tysig}
        and \tyvld{\concat{\AEnv}{\UEnv}}{\tysig'}, where \concat{\AEnv}{\UEnv}
...
\input{chapters/sec-dec-sub-ext.tex}
