...
Existential types are abstract.

Julia's existential types are \emph{impredicative}:
...
also describe type signatures of parametric method definitions.
...
It is not possible to directly invoke a parametric method definition with the
desired type argument. Instead, an instantiation of the type variable is found
when subtyping between a concrete argument type and existential method type
signature succeeds. \TODO{example}
Just like with Java wildcards, there are no explicit packing and
